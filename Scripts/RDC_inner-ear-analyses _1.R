#--------------------------------------------------------------
# Geometric morphometric analyses and tests for convergence of river dolphin inner ears
# Travis Park 2018
...
# Define number of discrete landmarks
fixed <- as.integer(1)  

# Define path to folder where the data are
mypath <- "Landmarks/L635-dataset/RDC_L635_pts"
...
slidematrix <- as.matrix(read.csv("Data/Sliding-matrix_1.csv"))

# Read in metadata on specimens
ds <- read.csv("Data/RDC_Habitat-type_2.csv")
...
gdf <- geomorph.data.frame(coords = RDC_gpa$coords, phy = cleanwhaletree, group = ds$Habitat)
gdf
...
RDC <- as.matrix(array(RDC_df[, 1:31]))
